/**
 * index.js
 * HTTP server entry point for OCRDS_AMS
 */
...
async function parseMultipartForm(req) {
  return new Promise((resolve, reject) => {
    ...
    const boundary = rawBoundary.replace(/^["\s]+|["\s;]+$/g, '').trim();
...
async function handleRequest(req, res) {
  ...
  if (url === '/health' && method === 'GET') {
    sendJSON(res, { status: 'ok', system: 'OCRDS_AMS', timestamp: new Date().toISOString() });
    return;
  }
  ...
  // Route: POST /process-text - DeepSeek only (OCR text from client)
  if (url === '/process-text' && method === 'POST') {
    try {
      if (!DEEPSEEK_API_KEY) {
        sendJSON(res, {
          error: 'DEEPSEEK_API_KEY not configured',
      ...
      req.on('end', async () => {
        try {
          ...
          const ocrText = parsed.ocr_text || parsed.ocrText || '';
          if (!ocrText || typeof ocrText !== 'string') {
            sendJSON(res, { success: false, error: 'Missing or invalid ocr_text' }, 400);
            return;
          }
          const structuredData = await processSafe(ocrText.trim(), DEEPSEEK_API_KEY);
          sendJSON(res, {
            success: true,
            structured_data: structuredData,
            processing_timestamp: new Date().toISOString()
          ...
          sendJSON(res, {
            ...
            error: err.message,
            timestamp: new Date().toISOString()
          }, err.message === 'Missing or invalid ocr_text' ? 400 : 500);
        }
      });
      req.on('error', () => {
        sendJSON(res, { success: false, error: 'Request error' }, 500);
      });
    } catch (error) {
      console.error('Process-text setup error:', error);
...
server.listen(PORT, () => {
  ...
  console.log(`  POST /process-text - DeepSeek only (body: { "ocr_text": "..." })`);
